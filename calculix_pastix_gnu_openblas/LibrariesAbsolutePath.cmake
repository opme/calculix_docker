...
macro(LIBRARIES_ABSOLUTE_PATH lib_list hints_paths)
  ...
  message(WARNING "lib_env: ${CMAKE_C_IMPLICIT_LINK_DIRECTORIES}")
  message(WARNING "lib_list: ${${lib_list}}")
  # copy the lib list 
  set (${lib_list}_COPY "${${lib_list}}")
  # reset the lib list to populate
  set(${lib_list} "")
  message(WARNING "${lib_list}_COPY")
  foreach(_library ${${lib_list}_COPY})
    message(WARNING library: "${_library}")
    if(EXISTS "${_library}")
      # if already an absolute path, nothing special to do
      message(WARNING "${library}")
      ...
      # try to find the lib
      message(WARNING "lib path ${lib_list} '${_library}' ${_library_path}")
      find_library(_library_path NAMES ${_library} HINTS ${hints_paths} ${_lib_env})
      if (_library_path)
          list(APPEND ${lib_list} ${_library_path})
      else()
	      message(WARNING "Dependency of ${lib_list} '${_library}' NOT FOUND")
      endif()
